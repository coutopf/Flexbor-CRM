(function () {
  ...
  function normalizeOpportunityPayload(payload, currentOpportunity) {
    const current = currentOpportunity && typeof currentOpportunity === 'object' ? currentOpportunity : {};
    const next = payload && typeof payload === 'object' ? { ...payload } : {};
    const estado = deriveOpportunityState(next.fase ?? current.fase);

    next.estado = estado;

    if (estado === 'aberta') {
      if (Object.prototype.hasOwnProperty.call(next, 'motivo_fecho')) {
        next.motivo_fecho = null;
      }
      return next;
    }

    const closeDate = next.data_fecho || current.data_fecho || todayIso();
    next.data_fecho = closeDate;

    if (Object.prototype.hasOwnProperty.call(next, 'motivo_fecho')) {
      next.motivo_fecho = next.motivo_fecho || current.motivo_fecho || null;
    }
